...
def main(args, override_args=None):
    ...
    if "datastore" not in global_vars():
        ...
        if knn_type in ["vanilla_knn_mt", "adaptive_knn_mt", "kernel_smoothed_knn_mt", "vanilla_knn_mt_visual", "robust_knn_mt", "skip_knn_mt"]:
    ...
    for subset in args.valid_subset.split(","):
        ...
        for i, sample in enumerate(progress):
            ...
            elif knn_type == "skip_knn_mt":
                non_pad_tokens, mask = filter_pad_tokens(sample["target"])
                datastore["vals"].add(non_pad_tokens)
                global_vars()["raw_target"] = sample["target"]
                datastore.set_pad_mask(mask)
                last_tokens = torch.zeros_like(sample["target"])
                last_tokens[:, 0] = 2
                last_tokens[:, 1:] = sample["target"][:, :-1]
                last_tokens = last_tokens.masked_select(mask)
                datastore["last"].add(last_tokens)
                max_target_positions = model_args.max_target_positions
                target_len = mask.sum(dim=-1)
                token_positions = []
                token_positions_tgt = []
                for tgt_len_, src_len_ in zip(target_len, sample['net_input']['src_lengths']):
                    # token_positions += [i+1 for i in range(len_)]
                    token_positions += [1-((2*(i+1))/max_target_positions) for i in range(tgt_len_)]
                    token_positions_tgt += [(i+1) for i in range(tgt_len_)]
                    # token_positions += [1-((2*np.log(i+1))/np.log(max_target_positions)) for i in range(len_)]
                token_positions = np.array(token_positions, dtype=np.float32)
                datastore["positions"].add(token_positions)
                token_positions_tgt = np.array(token_positions_tgt, dtype=np.float32)
                datastore["positions_tgt"].add(token_positions_tgt)
                sentence_ids = []
                src_sentence_ids = []
                for idx, sentence_id in enumerate(sample["id"].cpu().numpy()):
                    sentence_ids += [sentence_id]*target_len[idx]
                    src_sentence_ids.append(sentence_id)
                sentence_ids = np.array(sentence_ids, dtype=int)
                src_sentence_ids = np.array(src_sentence_ids, dtype=int)
                datastore["sentence_ids"].add(sentence_ids)
                datastore["src_sentence_ids"].add(src_sentence_ids)
                datastore["src_lengths"].add(sample['net_input']["src_lengths"])
            ## <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< end
    ...
    if knn_type in ["vanilla_knn_mt", "adaptive_knn_mt", "kernel_smoothed_knn_mt", "vanilla_knn_mt_visual", "plac_knn_mt", "robust_knn_mt", "skip_knn_mt"]:
        datastore.dump()    # dump to disk

        if args.valid_subset == 'train':
            datastore.build_faiss_index("keys", use_gpu=(not args.build_faiss_index_with_cpu))   # build faiss index
            if knn_type == "skip_knn_mt":
                datastore.build_faiss_index("src_keys", use_gpu=(not args.build_faiss_index_with_cpu))
