...
def _main(args, override_args, output_file):
    ...
    has_target = True
    wps_meter = TimeMeter()
    if args.batch_size is not None and args.batch_size == 1 and hasattr(models[0].decoder, 'retrieve_count'):
        ...
        for i, sample_id in enumerate(sample["id"].tolist()):
            ...
            for j, hypo in enumerate(hypos[i][: args.nbest]):
                ...
                if not args.quiet:
                    ...
                    if args.batch_size is not None and args.batch_size == 1 and \
                            hasattr(models[0].decoder, 'retrieve_count') and models[0].decoder.retrieve_count != []:
                        retrieve_count = sum(models[0].decoder.retrieve_count)
                        deno_count = sum(models[0].decoder.deno_count)
                        print(
                            "D-{}\t{}\t{}\t{}\t{}".format(sample_id, score, detok_hypo_str,
                                                          retrieve_count - last_retrieve_count, deno_count - last_deno_count),
                            file=output_file,
                        )
                        last_retrieve_count = retrieve_count
                        last_deno_count = deno_count
                    else:
                        print(
                            "D-{}\t{}\t{}".format(sample_id, score, detok_hypo_str),
                            file=output_file,
    ...
    if hasattr(models[0].decoder, 'retrieve_count') and models[0].decoder.retrieve_count != []:
        retrieve_count = sum(models[0].decoder.retrieve_count)
        deno_count = sum(models[0].decoder.deno_count)
        logger.info(f"During generation, retrieve number = {retrieve_count}")
        logger.info(f"During generation, total number = {deno_count}")
        skip_percent = 1 - retrieve_count / deno_count
        logger.info(
            "During generation, skip percent = {:.2f}%".format(skip_percent * 100)
        )

    if hasattr(models[0].decoder.retriever, "cache") and hasattr(models[0].decoder.retriever.cache, "hits"):
        hits = models[0].decoder.retriever.cache.hits
        total = models[0].decoder.retriever.cache.total

        print(
            "During generation, cache percent = {:.2f}%".format(hits / total * 100),
            file=output_file,
        )
    return scorer
