...
from faster_dataset import FasterDataset
from criterion import FasterCriterion
...
def main():
    ...
    # feature data
    data_list = ['query', 'target', 'knn_dist', 'knn_tgt']
    ...
    # train_dataset = FasterDataset(**valid_data_dict, right=split_point, move_data_to_gpu=args.move_data_to_gpu)
    # valid_dataset = FasterDataset(**valid_data_dict, left=split_point, move_data_to_gpu=args.move_data_to_gpu)
    dataset = FasterDataset(**valid_data_dict, move_data_to_gpu=args.move_data_to_gpu)
    ...
    optimizer = optim.Adam(model.parameters(), lr=1e-4, eps=1e-8, betas=[0.9, 0.98])
    criterion = FasterCriterion(
        with_adaptive=args.with_adaptive,
