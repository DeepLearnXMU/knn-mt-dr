...
def repetition_mask(index):
    bsz, beam = index.size()

    index_view = index.view(bsz, 1, beam).repeat(1, beam, 1)
    index_view_t = index_view.permute(0, 2, 1)

    mask = torch.tril(index_view == index_view_t).sum(dim=-1) == 1
    return mask