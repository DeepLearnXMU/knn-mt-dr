...
class RobustKNNMT(TransformerModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--robust-use-diff", default=False, action='store_true')
        parser.add_argument("--robust-use-term", default=False, action='store_true')

        parser.add_argument("--skip", action="store_true", default=False,
                            help="skip test")
        parser.add_argument("--skip-threshold", type=float, default=1,
                            help="skip threshold")
...
class RobustKNNMTDecoder(TransformerDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.update_num = 0
        self.robust_use_term = args.robust_use_term
        self.skip = args.skip
        ...
        if args.knn_mode == "build_datastore":
            ...
            if args.knn_mode == "train_metak":
                self.combiner = RobustCombiner(
                    ...
                    use_diff=args.robust_use_diff,
                )
            elif args.knn_mode == "inference":
                self.combiner = RobustCombiner.load(args.knn_combiner_path, args.robust_use_diff)
    ...
    def get_normalized_probs(
        ...
        if self.args.knn_mode == "inference" or self.args.knn_mode == "train_metak":
            ...
            if self.skip is True:
                retrieve_index = self.combiner.lambda_ > (1 - self.skip_threshold)
                self.combiner.lambda_ = torch.where(retrieve_index, self.combiner.lambda_, 0)
                total = retrieve_index.numel()
                retrieve_num = int(retrieve_index.sum())
                self.retrieve_count.append(retrieve_num)
                self.deno_count.append(total)
            combined_prob, _ = self.combiner.get_combined_prob(knn_prob, net_output[0], log_probs=log_probs)

            if self.robust_use_term:
                if log_probs:
                    network_probs = torch.log(network_probs)
                return combined_prob, network_probs, self.combiner.lambda_
            else:
...
@register_criterion("label_smoothed_cross_entropy_for_robust_term")
class LabelSmoothedCrossEntropyCriterionForRobustTerm(
    LabelSmoothedCrossEntropyCriterion
):
    def __init__(self, task, sentence_avg, label_smoothing, robust_term_alpha):
        super().__init__(task, sentence_avg, label_smoothing)
        self.alpha = robust_term_alpha
    ...
    def forward(self, model, sample, reduce=True):
        ...
        combined_lprobs, nmt_lprobs, knn_lambda = model.get_normalized_probs(net_output, log_probs=True)
        target = model.get_targets(sample, net_output)
        assert self.ignore_prefix_size == 0
        
        combined_lprobs = combined_lprobs.view(-1, combined_lprobs.size(-1))
        nmt_lprobs = nmt_lprobs.view(-1, nmt_lprobs.size(-1))
        knn_lambda, target = knn_lambda.view(-1), target.view(-1)
        ...
        nmt_top1_index = nmt_lprobs.topk(1, dim=-1).indices.squeeze(1)
        term_loss = torch.sum(knn_lambda * (target == nmt_top1_index).float())
        loss = loss + self.alpha * term_loss
