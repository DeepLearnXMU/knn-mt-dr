...
from knnbox.combiner import Combiner, SkipCombiner
...
class VanillaKNNMT(TransformerModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--specific-layer", type=int, default=None)
        parser.add_argument("--add-last-token", default=False, action="store_true")
        parser.add_argument("--add-token-position", default=False, action="store_true")
...
class VanillaKNNMTDecoder(TransformerDecoder):
    ...
    def forward(
        ...
        if self.specific_layer is None:
            query = x
        else:
            query = extra["specific_layer_features"]

        if self.args.knn_mode == "build_datastore":
            keys = select_keys_with_pad_mask(query, self.datastore.get_pad_mask())
