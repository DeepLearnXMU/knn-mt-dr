...
class FasterCombiner(nn.Module):
    def __init__(self, probability_dim, max_k, knn_lambda=0.7, temperature=10, mode=None,
        ...
        if self.mode == 'train':
            ...
            #self.output_projection = nn.Linear(512, probability_dim, bias=False)
            self.output_projection.load_state_dict(output_projection_ckp)

        self.meta_network = MetaNetwork(max_k=max_k, with_adaptive=with_adaptive)
    ...
    def get_knn_inf(self, knn_dist, knn_tgt):
        return self.meta_network.get_knn_inf(knn_dist, knn_tgt)
    ...
    def train_forward(
        ...
        knn_alpha = self.meta_network(query, is_train=True)
        if self.with_adaptive:
...
class MetaNetwork(nn.Module):
    def __init__(self, max_k, with_adaptive, midsize=32, dropout_rate=0.2):
        super().__init__()
        self.device = 'cuda'
        self.max_k = max_k
        self.with_adaptive = with_adaptive
    ...
    def forward(self, query, is_train=False):
        #query = self.bn(query)
        knn_alpha = self.alpha_func(query)
        if is_train:
            return knn_alpha
        return knn_alpha[..., 1]
